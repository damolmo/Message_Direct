...
class SettingsBanner extends StatelessWidget{
  ...
  Widget build(BuildContext context){
    return Container(
      ...
      height:  getDeviceWidth(context) > 580 ? getDeviceHeight(context) * 0.2 : getDeviceHeight(context) * 0.2,
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            height:  getDeviceWidth(context) > 580 ? getDeviceHeight(context) * 0.14 : getDeviceHeight(context) * 0.2,
            margin: EdgeInsets.only(
              top: getDeviceWidth(context) > 580 ? getDeviceHeight(context) * 0.08 : getDeviceHeight(context) * 0.07,
              bottom: getDeviceWidth(context) > 580 ? getDeviceWidth(context) * 0.0 : getDeviceHeight(context) * 0.05,
              left: getDeviceWidth(context) *  0.05,
            ),
            child: Text("Settings", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: getDeviceWidth(context) > 580 ? getDeviceWidth(context) * 0.05 : getDeviceWidth(context) * 0.13, shadows: getDeviceWidth(context) > 580 ? const [Shadow(color: Colors.black, offset: Offset.zero, blurRadius: 9.0)] : const [Shadow(color: Colors.black, offset: Offset.zero, blurRadius: 13.0)]),),
          )
        ],
      ),
    );
